<template>
    <div :class="{'my-input': true, 'my-input is-validated': checkInputLength(value),  'my-input not-validated': errors.has(label)}">
        ...
        <input type="password" :class="{'my-input__input' : true, 'my-input__input red-label': errors.has(label)}" :name="label" :value="value" :placeholder="label" v-validate="{ rules }" @input="updateField($event.target.value)"/>
